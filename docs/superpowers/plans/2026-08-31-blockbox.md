...
**Goal:** 4×4×14 직육면체 통 안으로 조각이 떨어지는 쿼터뷰 3D 테트리스를 Godot 4.6으로 만든다.
...
**Tech Stack:** Godot 4.6, GDScript. 외부 의존성 없음.
...
버전 문자열이 `4.6`으로 시작하는지 확인한다. brew가 다른 버전을 설치하면 https://godotengine.org/download 에서 4.6을 직접 받아 `/Applications/Godot.app`에 둔다.
...
func _test_bag_gives_each_kind_once() -> void:
	...
	for k in counts:
		assert(counts[k] >= 8, "조각 %d 가 너무 적게 나왔다: %d" % [k, counts[k]])
